...
from ann.ibmodel import FCN1_DB2 as ib_FCN1_DB2
from ann.ibmodel import FCN2_DB2 as ib_FCN2_DB2
...
if __name__ == '__main__':
    ...
    data = scio.loadmat('dataset/NinaPro-DB2/data2.mat')
    imageData = data['data']
    ...
    # network = InformationBottleneck(input_shape=(1, 16, 10), num_classes=52, network=ib_FCN1_DB2, K=args.K, beta=args.beta,
    #                                 ib_type='2OIB', logvar_t=args.logvar_t, train_logvar_t=args.train_logvar_t)
    #
    # network.fit(train_set, test_set, n_epochs=args.n_epochs, learning_rate=args.learning_rate,
    ...
    #
    # output_model = 'output/NonlinearIB_NinaPro-DB1_acc-0.7327126860618591.pkl'
    # network.network.load_state_dict(torch.load(output_model))
    # IXT, ITY, loss, performance = network.evaluate_network(test_loader, 3009)
    # print('IXT: {}, ITY: {}, loss: {}, performance: {}'.format(IXT, ITY, loss, performance))

    #
    ann = ib_FCN1_DB2(input_shape=(1, 16, 10), num_classes=52)
    ...
    ann.load_state_dict(torch.load('output/2OIB_-FCN2-NinaPro-DB2_acc-0.7599702477455139.pkl'), strict=True)
    ...
    # # print('sigma weight: ', snn.Weighted.data)
